        ...
        if (isItemInCart) {
            setCartItems(
                cartItems.map(
                    ...
                    ) => (cartItem.id === item.id ? { ...cartItem, quantity: cartItem.quantity + qty } : cartItem) // otherwise, return the cart item
                )
            );
        } else {
            setCartItems([...cartItems, { ...item, quantity: qty }]); // if the item is not in the cart, add the item to the cart
        }
    };
        ...
        if (isItemInCart) {
            setCartItems(
                cartItems.map(
                    (
                        cartItem // if the item is already in the cart, increase the quantity of the item
                    ) => (cartItem.id === item.id ? { ...cartItem, quantity: cartItem.quantity + 1 } : cartItem) // otherwise, return the cart item
                )
            );
        } else {
